...
import numpy as np
import librosa
from config.config_structure import get_insect_base_config

# Point to the repository root (this file lives at repo_root/audio_sync_generator.py)
...
def generate_insect_config():
    print("--- Generating insect config ---")
    insect_params = get_insect_base_config()
    for sp_id, params in insect_params.items():
        print(f"Processing insect: {sp_id}")
        chirp_patterns = {}
    ...
    out_path = os.path.join(PROJECT_ROOT, "config", "insect_config.py")
    print(f"\n--- Writing '{out_path}' (INSECT_PARAMS) ---")
    output_content = f"""# Auto-generated by audio_sync_generator.py (insect mode). DO NOT EDIT.
import numpy as np

INSECT_PARAMS = {format_python_code(insect_params, 4)}
"""
